...
public class RewardAPI {
    ...
    public ResponseEntity<List<Reward>> getRewardsByCustomerId(@RequestParam Long customerId) {
        List<Reward> rewards = rewardService.getRewardsByCustomerId(customerId);
        return ResponseEntity.ok(rewards);
    }

    @GetMapping("/rewardClaimed")
    public ResponseEntity getAllRewardClaimed(@RequestParam int page, @RequestParam(defaultValue = "10") int size) {
        RewardClaimedResponse rewardClaimedResponse = rewardService.getAllRewardClaimed(page, size);
        return ResponseEntity.ok(rewardClaimedResponse);
    }
